...
#### 全球单播地址的等级结构
+ `IPv6`把1/8的地址空间划分为`全球单播地址`；
![image](https://github.com/ningbaoqi/ComputerNetWork/blob/master/gif/pic-275.jpg)

|字段|说明|
|------|------|
|全球路由选择前缀|分配给各公司和机构，用于因特网中路由器的路由选择，这相当于最初分配的IPv4地址中的网络号字段|
|子王标识符|用于各公司和机构创建自己的子网，对于小公司，可以把这个字段设置为全0|
|接口标识符|指明主机或路由器单个的网络接口，实际上就相当于分类的IPv4地址中的主机号字段|

+ 为了保证可操作性，所有的计算机都必须对硬件地址使用相同的编码方式，定义了一个标准64位全球唯一地址格式`EUI-64`，当一个`EUI-64`硬件地址需要转换为`IPv6`地址时，只要把它放入IPv6地址中的接口标识符字段中即可，但要把公司标识符的第一个字节的最低第二位进行`取反`，即把1转换成0，而把0转换成1；
### 从IPv4向IPv6过渡
#### 双协议桟
+ 双协议桟是指在完全过去到IPv6之前，使一部分主机或路由器装两个协议桟，一个IPv4和一个IPv6，因此双协议桟主机或路由器既能和IPv6的系统通信，又能和IPv4的系统进行通信，双协议桟的主机或路由器记为IPv6/IPv4，表明它具有两种IP地址：一个IPv6地址和一个IPv4地址；
![image](https://github.com/ningbaoqi/ComputerNetWork/blob/master/gif/pic-276.jpg)
#### 隧道技术
+ IPv6数据报要进入IPv4网络时，将IPv6数据报封装成为IPv4数据报(整个的IPv6数据报变成了IPv4数据报的数据部分)，然后，IPv6数据报就在IPv4网络的隧道中传输，当IPv4数据报离开IPv4网络中的隧道时再把数据部分(原来的IPv6数据报)交给主机的IPv6协议桟；
![image](https://github.com/ningbaoqi/ComputerNetWork/blob/master/gif/pic-277.jpg)
### ICMPv6
+ 在新版本的网络层中，地址解析协议ARP和网际组管理协议IGMP协议的功能都已被合并到ICMPv6中；
![image](https://github.com/ningbaoqi/ComputerNetWork/blob/master/gif/pic-278.jpg)
![image](https://github.com/ningbaoqi/ComputerNetWork/blob/master/gif/pic-279.jpg)
+ ICMPv6报文中的信息报文有两种：`回送请求报文`和`回送回答报文`，回送请求和回送回答报文被设计用于检测因特网上的两个设备之间是否能够互相通信；ICMPv6的邻站发现报文需要使用两个新协议：`邻站发现协议ND`和`反向邻站发现协议IND`：主机使用ND协议来发现能够为其转发分组的邻居路由器，结点使用ND协议来发现邻站的链路层协议(相连在同一个网络上的结点)，结点使用IND协议来发现邻站的IPv6地址；ICMPv6的邻站发现报文有以下几种：`路由器询问报文`用来发现网络中能够为该主机转发IPv6报文的路由器；`路由器通告报文`响应路由器询问报文；`邻站询问报文`与ARP请求报文的任务一样，`邻站通告报文`响应邻站询问报文，`改变路由报文`(与版本4一样)，`反向邻站询问报文`知道了邻站的链路层地址，但是需要知道该邻站IP地址，`反向邻站通告报文`响应反响邻站询问报文；ICMPv6的组成员关系报文有两种：`成员关系查询报文`由路由器发送的，目的是为了找出网络中活跃的组成员；`成员关系报告报文`；
